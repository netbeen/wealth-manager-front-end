...
export const fetchCurrentOrganizationWithPermission: () => Promise<OrganizationPermissionType> = async () => {
  const result = (
    await axios.get(`${API_PREFIX}/organization/current`, {
...
export const fetchAvailableOrganizations: () => Promise<
  Array<OrganizationPermissionType['organization']>
> = async () => {
  const availableOrganizationsResult = await axios.get(
    `${API_PREFIX}/organization/getAvailableOrganizations`,
    {
      headers: getAuthorizationHeaders(),
    },
  );
