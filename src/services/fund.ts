 ...
 * @param identifier
 */
export const fetchBasicInfo: (identifier: string) => Promise<FundBasicInfoType> = async (
  identifier,
) => {
  const result = (
    await axios.get(`${API_PREFIX}/fund/basicInfo`, {
      params: {
        identifier,
      },
    })
...
export const fetchUnitPriceByIdentifier: (
  identifier: string,
) => Promise<Array<FundPriceType>> = async (identifier) => {
  const result = (
    await axios.get(`${API_PREFIX}/fund/unitPrice`, {
      params: {
        identifier,
      },
    })
...
export const fetchBasicInfoUnitPriceSplitDividendByIdentifier: (
