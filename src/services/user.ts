...
export const fetchMe: () => Promise<UserType> = async () => {
  const result = (
    await axios.get(`${API_PREFIX}/user/me`, {
...
export const register: (username: string, passwordHash: string) => Promise<UserType> = async (
  username,
  passwordHash,
) => {
  const result = (
...
export const login: (username: string, passwordHash: string) => Promise<string> = async (
  username,
  passwordHash,
) => {
  const result = (
