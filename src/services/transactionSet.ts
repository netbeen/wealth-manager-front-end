...
export const fetchTransactionSetsByStatus: (
  ...
  const result = (
    await axios.get(`${API_PREFIX}/fund/transactionSet`, {
...
export const fetchTransactionSetById: (id: string) => Promise<TransactionSetType | null> = async (
  id,
) => {
  const result = (
    await axios.get(`${API_PREFIX}/fund/getTransactionSetById`, {
      headers: getAuthorizationHeaders(),
      params: {
        id,
      },
